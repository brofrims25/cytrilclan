...
public class ClanCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!player.hasPermission("cytrilclan.use")) {
            MessageUtil.sendError(player, "You don't have permission to use clan commands.");
            return true;
        }
        ...
        if (args.length == 0) {
            ...
            player.openInventory(MainMenuGui.build(clan, player, plugin.getConfigManager()));
            plugin.getSoundUtil().play(player, "gui-open");
            return true;
        }
        ...
        switch (sub) {
            ...
            case "bank" -> {
                if (!requireFeature(player, plugin.getConfigManager().isBankEnabled(), "The clan bank")) break;
                openSub(player, () -> com.cytril.cytrilclan.gui.BankGui.build(requireClan(player)));
            }
    ...
    /** Returns true if the feature is enabled; otherwise sends an error and returns false. */
    private boolean requireFeature(Player player, boolean enabled, String featureLabel) {
        if (!enabled) {
            MessageUtil.sendError(player, featureLabel + " is disabled on this server.");
        }
        return enabled;
    }
    ...
    private void handleCreate(Player player, String[] args) {
        ...
        if (!name.matches("[A-Za-z0-9_]{3,16}")) {
            MessageUtil.sendError(player, "Clan names must be 3-16 characters: letters, numbers, underscore only.");
            return;
        }
        if (!tag.matches("[A-Za-z0-9]{2,5}")) {
            MessageUtil.sendError(player, "Clan tags must be 2-5 characters: letters and numbers only.");
            return;
        }
        ...
        int maxClans = plugin.getConfigManager().getMaxClans();
        if (maxClans > 0 && plugin.getClanManager().getAllClans().size() >= maxClans) {
            MessageUtil.sendError(player, "The server has reached the maximum number of clans (" + maxClans + ").");
            return;
        }
        double cost = plugin.getConfigManager().getClanCreationCost();
        if (plugin.getConfigManager().isEconomyEnabled() && cost > 0) {
            var vault = plugin.getVaultHook();
            if (vault.isAvailable()) {
                if (!vault.has(player, cost)) {
                    MessageUtil.sendError(player, "You need " + vault.format(cost) + " to create a clan.");
                    return;
                }
                if (!vault.withdraw(player, cost)) {
                    MessageUtil.sendError(player, "Payment failed. Please try again.");
                    return;
                }
                MessageUtil.send(player, "&7Charged " + vault.format(cost) + " for clan creation.");
            }
        }
    ...
    private void handleInvite(Player player, String[] args) {
        ...
        if (maxMembers > 0 && clan.getSize() >= maxMembers) {
            MessageUtil.sendError(player, "Your clan already has the maximum number of members (" + maxMembers + ").");
            return;
        }
        ...
        MessageUtil.send(target, "&f" + player.getName() + " invited you to join &6" + clan.getName() + "&f! Use &e/clan join " + clan.getName() + "&f within " + plugin.getConfigManager().getInviteExpiryMinutes() + " minutes to accept.");
    }
    ...
    private void handleJoin(Player player, String[] args) {
        ...
        long expiryMs = plugin.getConfigManager().getInviteExpiryMinutes() * 60L * 1000L;
        if (args.length < 2) {
            var invites = plugin.getClanManager().getPendingInvitesFor(player.getUniqueId(), expiryMs);
            if (invites.isEmpty()) {
                MessageUtil.send(player, "&7You have no pending clan invites right now.");
                return;
            }
            MessageUtil.sendRaw(player, "&8&m----&r &6Pending Invites &8&m----");
            for (Clan invitingClan : invites) {
                MessageUtil.sendRaw(player, "&f" + invitingClan.getName() + " &8[" + invitingClan.getFormattedTag() + "&8] &7- &e/clan join " + invitingClan.getName());
            }
            return;
        }
        ...
        if (System.currentTimeMillis() - invitedAt > expiryMs) {
            MessageUtil.sendError(player, "That invite expired. Ask an officer or the leader to invite you again.");
            return;
        }
        int maxMembers = plugin.getConfigManager().getMaxMembersPerClan();
        if (maxMembers > 0 && clan.getSize() >= maxMembers) {
            MessageUtil.sendError(player, "That clan is already full.");
            return;
        }
    ...
    private void handleKick(Player player, String[] args) {
        ...
        if (args.length < 2) {
            MessageUtil.sendError(player, "Usage: /clan kick <player> [reason]. Tip: use the GUI (shift-click Kick) for a book reason.");
            return;
        }
        ClanMember actor = clan.getMember(player.getUniqueId());
        ...
        MessageUtil.sendSuccess(player, target.getLastKnownName() + " was kicked." + (reason != null ? " Reason: " + reason : ""));
        Player kicked = Bukkit.getPlayer(target.getUuid());
        if (kicked != null) {
            MessageUtil.sendError(kicked, "You were kicked from " + clan.getName() + "." + (reason != null ? " Reason: " + reason : ""));
        }
    }
    ...
    private void handleDisband(Player player, String[] args) {
        ...
        if (args.length < 2 || !args[1].equalsIgnoreCase("confirm")) {
            MessageUtil.sendError(player, "This can't be undone. Type &f/clan disband confirm&c to proceed, or use the menu (/clan) instead.");
            return;
        }
        ...
        for (UUID uuid : clan.getMembers().keySet()) {
            ...
            if (member != null && !member.equals(player)) {
                MessageUtil.sendError(member, "Your clan " + name + " was disbanded by the leader.");
            }
        }
        refundOnDisband(player);
        plugin.getClanManager().disbandClan(clan);
        MessageUtil.sendSuccess(player, "Clan disbanded.");
    }

    /** Refunds a configurable percentage of the clan-creation cost to the leader on disband, if enabled. */
    private void refundOnDisband(Player leader) {
        if (!plugin.getConfigManager().isEconomyEnabled() || !plugin.getConfigManager().isRefundOnDisband()) {
            return;
        }
        var vault = plugin.getVaultHook();
        if (!vault.isAvailable()) {
        ...
        double cost = plugin.getConfigManager().getClanCreationCost();
        int percent = plugin.getConfigManager().getRefundPercent();
        double refund = cost * percent / 100.0;
        if (refund > 0 && vault.deposit(leader, refund)) {
            MessageUtil.send(leader, "&7Refunded " + vault.format(refund) + " for disbanding.");
        }
    }

    private void handleBase(Player player, String[] args) {
        if (!requireFeature(player, plugin.getConfigManager().isBasesEnabled(), "Clan bases")) return;
        Clan clan = requireClan(player);
        if (clan == null) {
            MessageUtil.sendError(player, "You're not in a clan.");
            return;
        ...
        if (action.equals("set")) {
            ...
            if (!baseName.matches("[A-Za-z0-9 _-]{2,24}")) {
                MessageUtil.sendError(player, "Base names must be 2-24 characters (letters, numbers, spaces, - or _).");
                return;
            }
    ...
    private void handleInfo(Player player, String[] args) {
        ...
        MessageUtil.sendRaw(player, "&8&m----&r " + clan.getFormattedName() + " &8[" + clan.getFormattedTag() + "&8] &8&m----");
    ...
    private void handleList(Player player) {
        ...
        for (Clan clan : clans) {
            MessageUtil.sendRaw(player, clan.getFormattedName() + " &8[" + clan.getFormattedTag() + "&8] &7- " + clan.getSize() + " member(s)");
        }
    }
    ...
    private void handleTransfer(Player player, String[] args) {
        ...
        if (args.length < 2) {
            MessageUtil.sendError(player, "Usage: /clan transfer <player>. Use /clan transfer cancel to cancel a pending transfer.");
            return;
        }
        if (args[1].equalsIgnoreCase("cancel")) {
            if (plugin.getClanManager().cancelPendingLeaderTransfer(clan)) {
                MessageUtil.sendSuccess(player, "Pending leadership transfer cancelled.");
            } else {
                MessageUtil.sendError(player, "There's no pending leadership transfer.");
            }
            return;
        }
        ClanMember target = null;
        ...
        if (target.getUuid().equals(player.getUniqueId())) {
            MessageUtil.sendError(player, "You're already the leader.");
            return;
        }
        if (plugin.getClanManager().hasPendingLeaderTransfer(clan)) {
            MessageUtil.sendError(player, "There's already a pending transfer. Use /clan transfer cancel first.");
            return;
        }
        int delayMinutes = plugin.getConfigManager().getLeaderTransferDelayMinutes();
        var task = new com.cytril.cytrilclan.tasks.LeaderTransferTask(plugin, clan, target.getUuid());
        ...
        MessageUtil.sendSuccess(player, "Leadership will transfer to " + target.getLastKnownName() + " in " + delayMinutes + " minute(s). Use /clan transfer cancel to stop it.");
    }

    private void handleGive(Player player) {
        if (!requireFeature(player, plugin.getConfigManager().isGiveItemEnabled(), "Giving items")) return;
        Clan clan = requireClan(player);
        if (clan == null) {
            MessageUtil.sendError(player, "You're not in a clan.");
            return;
    ...
    private void handlePvp(Player player, String[] args) {
        if (!requireFeature(player, plugin.getConfigManager().isPvpToggleEnabled(), "Clan PVP toggle")) return;
        Clan clan = requireClan(player);
        if (clan == null) {
            MessageUtil.sendError(player, "You're not in a clan.");
            return;
        ...
        if (args.length < 2 || (!args[1].equalsIgnoreCase("on") && !args[1].equalsIgnoreCase("off"))) {
            MessageUtil.send(player, "&7Clan PVP is currently " + (member.isPvpEnabled() ? "&aON" : "&cOFF") + "&7. Usage: /clan pvp on|off");
            return;
        }
        boolean enable = args[1].equalsIgnoreCase("on");
        if (member.isPvpEnabled() == enable) {
            MessageUtil.sendError(player, "Clan PVP is already " + (enable ? "on" : "off") + " for you.");
            return;
        }
        member.setPvpEnabled(enable);
        plugin.getClanManager().save(clan);
        if (enable) {
            MessageUtil.sendSuccess(player, "Clan PVP is now ON - your clanmates can hit you and you can hit them again.");
        } else {
            MessageUtil.sendSuccess(player, "Clan PVP is now OFF - your clanmates can no longer hit you, and you can't hit them. This has no effect on other clans or clanless players.");
        }
    }
    ...
    private void sendHelp(Player player) {
        ...
        MessageUtil.sendRaw(player, "&e/clan join [name] &7- list your pending invites, or accept one");
        MessageUtil.sendRaw(player, "&e/clan leave &7- leave your clan");
        MessageUtil.sendRaw(player, "&e/clan kick <player> [reason] &7- kick a member");
        MessageUtil.sendRaw(player, "&e/clan disband confirm &7- disband your clan (leader only)");
        ...
        MessageUtil.sendRaw(player, "&e/clan transfer <player>|cancel &7- transfer leadership (delayed, cancellable)");
        MessageUtil.sendRaw(player, "&e/clan give &7- give an item to a member");
        MessageUtil.sendRaw(player, "&e/clan pvp on|off &7- toggle being hittable by your own clanmates");
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> subs = List.of("create", "invite", "join", "leave", "kick", "disband", "base",
                    "bank", "settings", "info", "list", "promote", "demote", "transfer", "give", "pvp", "help");
            String partial = args[0].toLowerCase(Locale.ROOT);
            return subs.stream().filter(s -> s.startsWith(partial)).collect(Collectors.toList());
        }
        if (args.length == 2 && args[0].equalsIgnoreCase("join") && sender instanceof Player joinPlayer) {
            long expiryMs = plugin.getConfigManager().getInviteExpiryMinutes() * 60L * 1000L;
            return plugin.getClanManager().getPendingInvitesFor(joinPlayer.getUniqueId(), expiryMs).stream()
                    .map(Clan::getName)
        ...
        if (args.length == 2 && args[0].equalsIgnoreCase("disband")) {
            return List.of("confirm").stream().filter(s -> s.startsWith(args[1].toLowerCase(Locale.ROOT))).collect(Collectors.toList());
        }
        if (args.length == 2 && args[0].equalsIgnoreCase("transfer")) {
            List<String> options = new ArrayList<>(List.of("cancel"));
            Bukkit.getOnlinePlayers().forEach(p -> options.add(p.getName()));
            return options.stream().filter(s -> s.toLowerCase(Locale.ROOT).startsWith(args[1].toLowerCase(Locale.ROOT))).collect(Collectors.toList());
        }
